...
**Status:** MVP Complete (Phases 0-8 done — see §24 Definition of Done), post-MVP Settings/Network IP (§29) and Internal Server (§30) added, deployed and running on a real Raspberry Pi (see DEPLOY_PLAN.md)  
...
# 30. Internal Server (`internal-server/`)

Added during Raspberry Pi deployment Session 3 (2026-08-26), on direct request: the real, always-on MQTT consumer this project's forwarder (§15, `MQTTAdapter`) had always been designed to talk to. `cmd/server-sim` (§15, Phase 4/5) was always explicitly dev-only and in-memory — this closes that gap for real.

**What triggered it**: mid-Session-3, the Pi's gateway was found stuck for 4.5 hours with a growing backlog and every batch timing out waiting for an ack. Investigation (real `mosquitto_pub`/`mosquitto_sub` against the broker, subscribing to the gateway's own data topic directly) showed the broker and the gateway's publish path were both fine — the batches were arriving on `gateway/GW001/data` exactly as expected. The actual cause: nothing was subscribed to consume them and publish the application-level ack back, because `cmd/server-sim` (the only prior consumer, run manually during Session 2's testing) was not running. `mqtt.nxge.co` had no permanent consumer at all. See `DEPLOY_PLAN.md`'s Session 3 log for the full diagnostic trail, including one real `mqttadapter.go` `onConnect` gap found along the way (ack-topic re-subscribe has no retry on failure) that turned out not to be this incident's cause but is still worth fixing.

**Scope decision**: rather than leave `cmd/server-sim` running as a permanent stand-in (explicitly against its own doc comment — "never deployed to production"), a real service was built from scratch: `internal-server/`, a separate Go module and Docker Compose stack, deliberately outside `gateway/` since it's conceptually a different deployment (the downstream consumer, not the edge device).

Implementation:
- **`internal-server/consumer.go`** — subscribes `gateway/+/data` (a wildcard, not hardcoded to one gateway ID, so the same service can consume from a fleet later — see §25's fleet management roadmap item). On each batch: persists every entry, then publishes the ack back to `strings.TrimSuffix(topic, "/data") + "/ack"` — deriving the ack topic from the actual topic received rather than hardcoding it, so it works for whichever gateway ID published. Unlike the gateway-side `onConnect` this exists to compensate for, the re-subscribe-on-reconnect here **does** retry on failure (`subscribeWithRetry`, 3 attempts with a short backoff) — written specifically in response to the gap found in `mqttadapter.go` during this session's incident.
- **`internal-server/store.go`** — Postgres (`pgx/v5`), one `readings` table, `UNIQUE(gateway_id, sequence_id)` as the idempotency key — the same dedup contract the gateway's own `data_queue`/Store & Forward design already uses (§6's Rule 6), so a retried batch after a lost ack never double-counts. `ON CONFLICT DO NOTHING` plus `RowsAffected()` distinguishes newly-accepted rows from duplicates for logging.
- **`internal-server/server.go`** — a small JSON API (`/health`, `/latest` — one row per gateway/device/datapoint combination, newest-first via `DISTINCT ON`, `/readings` — filterable by gateway/device/datapoint, `/stats` — per-gateway counts) plus a `/dashboard` page (also served at `/`) built as a single embedded (`go:embed`) HTML file with vanilla JS — no separate frontend build step, deliberately, since this is a small ops tool rather than a second product surface. Auto-refreshes every 5s; stat tiles include an inline SVG sparkline of the last 30 readings per datapoint.
- **`internal-server/docker-compose.yml`** — two services, `internal-server` and `postgres:16-alpine` (with a healthcheck gating startup order and a named volume for durability). Points at the real `mqtt.nxge.co` broker by default via `MQTT_BROKER_URL`, not the dev-only `mosquitto` service in the root `docker-compose.yml` — these are two separate brokers serving two separate purposes (one for this project's own Docker dev loop, one for the real deployed Pi).

**Verified live**: built, deployed via `docker compose up -d --build` (currently running on the Windows dev machine, not yet an always-on host — see HANDOFF.md's "Next"), confirmed connecting to `mqtt.nxge.co` and ingesting real batches from the Pi's gateway (`mqtt: connected`, `ingested batch ... accepted=N duplicates=0` in its logs), confirmed the Pi's backlog drains to 0 and `server_connected` flips to `true` with this running, and confirmed the dashboard renders real data (stat tiles, sparklines, and the readings table all populated from the live Pi).

**Known gaps**: no TLS/auth on the MQTT connection itself (matches the broker's current plain-TCP-no-auth setup, a separate concern from this service); Postgres credentials are dev-shaped defaults (`internal_server`/`internal_server`); not deployed to an always-on host yet, so if the Windows machine/Docker stops, the Pi's backlog simply grows again (harmlessly, by design — Rule 1) until it's restarted; no device/datapoint name metadata is carried over the wire (`WireEntry` only has numeric `device_id`/`datapoint_id`), so the dashboard labels tiles by ID rather than by human-readable name — joining against the gateway's own device/datapoint API would be a natural follow-up if this becomes a longer-lived tool.
