...
- **Historical readings from before the 2026-08-27 `internal-server/` move are not on the new host** — they remain in the `internal-server_internal-server-db` Docker volume on the Windows laptop (stopped, not deleted). Migrate with `pg_dump`/`pg_restore` if that history is ever needed; otherwise it can stay archived indefinitely or be cleaned up.
