...
Everything through the Session 4 work (internal-server host move, MQTT resubscribe retry) is committed and pushed to `origin/master` (https://github.com/bpp-pico/nxIIoT-Gateway.git). As of this handoff there are **uncommitted Session 4/5 doc updates** on the Windows dev machine (`.gitignore`, `DEPLOY_PLAN.md`, `HANDOFF.md`, `README.md`, `MEMORY.md`, `spec.md`, `internal-server/static/dashboard.html`) — run `git status`/`git diff --stat` for the current, authoritative set rather than trusting this list once more work has happened. Run `git log --oneline` for commit history rather than trusting a commit list copied into this doc (it goes stale fast). Git committer identity: `bpp-pico <banpot@pico.co.th>`.
...
| post-MVP | **Internal Server** (§30): the real, always-on MQTT consumer `cmd/server-sim` was always a stand-in for — subscribes `gateway/+/data`, dedupes into Postgres on `gateway_id`+`sequence_id`, acks back, serves a live dashboard (in-page Health/Stats/Latest/Readings nav, added Session 5) + JSON API | `internal-server/` (separate Go module, own Docker Compose stack — not part of `gateway/`) |
...
5. **`docker compose up -d --build` reused a stale cached layer for a `go:embed`'d static-file-only change** (`internal-server/static/dashboard.html`, Session 5, 2026-08-27) — the `COPY . .` build step reported `CACHED` even though the file had genuinely changed, leaving the container serving old content with no error anywhere. Only caught because verification checked the live served content directly instead of trusting the build log. Fix: `docker compose build --no-cache <service> && docker compose up -d --force-recreate <service>` — now the standard way to deploy any `internal-server/`-only change, not a fallback. See MEMORY.md.

**Takeaway for future work on this repo**: after unit tests pass, do a live `docker compose` run — and for anything touching failure/recovery paths specifically, an actual chaos test (`docker network disconnect`, `docker compose kill -s SIGKILL`, stopping a real dependency) — before considering a change done. This has found real, distinct bugs five times now, not hypothetically.
